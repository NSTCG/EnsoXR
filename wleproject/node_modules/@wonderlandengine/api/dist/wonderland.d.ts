...
import { Resource, ResourceType, SceneResource } from './resources/resource.js';
 ...
 * @since 1.5.0
 */
export declare enum GraphicsApi {
    /** WebGL 2.0 */
    WebGL2 = 0,
    /** WebGPU */
    WebGPU = 1
...
export declare class CollisionComponent extends Component {
     ...
     *
     * @param b Whether this collider is rendered in the debug
     *     visualization.
     * @since 1.5.0
     */
    set visualize(b: boolean);
    /**
...
export declare class ViewComponent extends Component {
    ...
    /**
     * Get the view space direction towards a 2D point in HTML canvas
     * coordinates.
     *
     *  Example:
     *
     ...
     *
     * @param x X coordinate between 0 and {@link viewport} width (exclusive),
     *     increasing values go right.
     * @param y Y coordinate between 0 and {@link viewport} height (exclusive),
     *     increasing values go **down**.
     * @param out Destination array/vector, expected to have at least 3 elements.
     * @returns The `out` parameter.
     * @since 1.5.0
     */
    getDirectionForScreenPosition<T extends NumberArray>(x: number, y: number, out: T): T;
}
/**
...
export declare enum ParticleReplacementPolicy {
    /** Don't replace existing particles. */
    DontReplace = 0,
...
export declare class ParticleEffectComponent extends Component {
     ...
     * Spawn new particles.
     *
     * The spawned particles will always be at the end of the alive range so
     * you can use {@link aliveCount} and the return value to get the index
     * range:
     *
     * ```js
     * const count = comp.spawn(10, 5.0);
     * const end = comp.aliveCount;
     ...
     * ```
     *
     * Modifies values in {@link lifetimes}. Manually changing lifetimes will
     * stop {@link spawn} and {@link aliveCount} from working correctly.
     *
     * When particles expire or get replaced, remaining particles will get
     * moved to ensure a contiguous range of alive particles. Use {@link ids}
     * to track individual particles.
     *
     * @param count Number of particles to spawn.
     * @param lifetime Lifetime of the spawned particles, in seconds. If `0`,
     *     the particles don't expire.
     * @param replacementPolicy Policy to handle spawning when the maximum
     *     particle count is reached.
     * @returns Number of spawned particles. Can be less than `count` if the
     *     maximum particle count was reached and the replacement policy is
     *     {@link ParticleReplacementPolicy.DontReplace}.
     */
    spawn(count?: number, lifetime?: number, replacementPolicy?: ParticleReplacementPolicy): number;
    /**
     * Number of particles spawned with {@link spawn} and still alive (i.e.
     * their lifetime hasn't expired yet or was set to `0`).
     *
     * Can be passed to {@link update}.
     */
    get aliveCount(): number;
    /**
     * Get persistent IDs.
     *
     * When using {@link spawn}, particle indices may change as particles
     * expire. You can use IDs as persistent indices into external data or as
     * seeds for generating random data per particle.
     *
     * ID range is between 0 and {@link ParticleEffect.maxCount}. Values are
     * unique for each particle within the component, but not globally.
     *
     * @returns Array of IDs or `null`, if {@link particleEffect} is not set.
     */
    get ids(): Readonly<NumberArray> | null;
    /**
     * Get an attribute accessor to modify particle transforms.
     *
     * Transforms are in local space, analogous to
     * {@link Object3D.setTransformLocal}.
     *
     * @returns Transform accessor or `null`, if {@link particleEffect} is not
     *     set.
     *
     * Call {@link update} for changes to take effect.
     */
    get transforms(): AttributeAccessor<Float32ArrayConstructor> | null;
    /**
     * Get an attribute accessor to modify particle scalings.
     *
     * Scalings are in local space, analogous to
     * {@link Object3D.setScalingLocal}.
     *
     * @returns Scaling accessor or `null`, if {@link particleEffect} is not set.
     *
     * Call {@link update} for changes to take effect.
     */
    get scalings(): AttributeAccessor<Float32ArrayConstructor> | null;
    /**
     * Get an attribute accessor to read or modify particle lifetimes.
     *
     * First component is the elapsed lifetime, second component the total
     * lifetime the particle was created with. Both are in seconds.
     *
     * For convenient lifecycle management, prefer using {@link spawn} and
     * {@link aliveCount} instead of manually modifying lifetimes. Changing
     * lifetimes manually will stop {@link spawn} and {@link aliveCount} from
     * working correctly.
     *
     * @returns Lifetime accessor or `null`, if {@link particleEffect} is not set.
     *
     * Call {@link update} for changes to take effect.
     */
    get lifetimes(): AttributeAccessor<Float32ArrayConstructor> | null;
    /**
     * Get an attribute accessor to modify particle colors.
     *
     * @returns Color accessor or `null`, if {@link particleEffect} is not set.
     *
     * Call {@link update} for changes to take effect.
     */
    get colors(): AttributeAccessor<Float32ArrayConstructor> | null;
    /**
     * Get an attribute accessor to modify particle instance data.
     *
     * @returns Instance data accessor or `null`, if
     *     {@link particleEffect} is not set.
     *
     * Call {@link update} for changes to take effect.
     */
    get instanceData(): AttributeAccessor<Float32ArrayConstructor> | null;
    /**
     * Apply changes to {@link transforms}, {@link scalings},
     * {@link lifetimes}, {@link colors} and {@link instanceData}.
     *
     * Uploads the updated particle attributes to the GPU.
     *
     * Since this is an expensive operation, call it only once you have
     * performed all modifications on a particle effect and avoid calling if
     * you did not perform any modifications at all.
     *
     * @param count Number of particles to update and render, clamped to
     *     {@link ParticleEffect#maxCount}.
     */
...
export declare class PhysXComponent extends Component {
    ...
    get trigger(): boolean;
    /**
     * Set whether this physics body is rendered in the debug visualization.
     *
     * @param b Whether this physics body is rendered in the debug
     *     visualization.
     * @since 1.5.0
     */
    set visualize(b: boolean);
    /**
     * Whether this physics body is rendered in the debug visualization.
     *
     * @since 1.5.0
     */
    get visualize(): boolean;
    ...
    get mass(): number;
    /**
     * Set mass space inertia tensor.
     *
     * [PhysX Manual - "Mass Properties"](https://gameworksdocs.nvidia.com/PhysX/4.1/documentation/physxguide/Manual/RigidBodyDynamics.html#mass-properties)
     *
     * Has no effect, if the component is not active.
     *
     * @param v New mass space inertia tensor.
     */
    set massSpaceInertiaTensor(v: Readonly<NumberArray>);
    /**
     * @deprecated Use {@link massSpaceInertiaTensor} instead.
     */
 ...
 * #### Update
 *
 * To modify a mesh, you get access to a {@link AttributeAccessor} that
...
export declare class Mesh extends Resource {
    ...
    /** @overload */
    attribute(attr: MeshAttribute.Position): AttributeAccessor<Float32ArrayConstructor> | null;
    /** @overload */
    attribute(attr: MeshAttribute.Tangent): AttributeAccessor<Float32ArrayConstructor> | null;
    /** @overload */
    attribute(attr: MeshAttribute.Normal): AttributeAccessor<Float32ArrayConstructor> | null;
    /** @overload */
    attribute(attr: MeshAttribute.TextureCoordinate): AttributeAccessor<Float32ArrayConstructor> | null;
    /** @overload */
    attribute(attr: MeshAttribute.Color): AttributeAccessor<Float32ArrayConstructor> | null;
    /** @overload */
    attribute(attr: MeshAttribute.JointId): AttributeAccessor<Uint16ArrayConstructor> | null;
    /** @overload */
    attribute(attr: MeshAttribute.JointWeight): AttributeAccessor<Float32ArrayConstructor> | null;
    /** @overload */
    attribute(attr: MeshAttribute.SecondaryTextureCoordinate): AttributeAccessor<Float32ArrayConstructor> | null;
    /**
     * Get an attribute accessor to retrieve or modify data of given attribute.
     *
     ...
     */
    attribute(attr: MeshAttribute): AttributeAccessor | null;
    /**
...
/**
 * Options to create a new {@link AttributeAccessor} instance.
 */
interface AttributeAccessorOptions<T extends TypedArrayCtor> {
    attribute: number;
    offset: number;
    stride: number;
    formatSize: number;
 ...
 * ```
 */
export declare class AttributeAccessor<T extends TypedArrayCtor = TypedArrayCtor> {
     ...
     * @hidden
     */
    constructor(engine: WonderlandEngine, options: AttributeAccessorOptions<T>);
    /**
...
export { AttributeAccessor as MeshAttributeAccessor };
/**
...
/**
 * Particle effect constructor parameters object.
 *
 * Usage:
 *
 * ```js
 * const particleEffect = ParticleEffect({maxCount: 10, mesh: myMesh, material: myMaterial});
 * ```
 */
export interface ParticleEffectParameters {
    /** Number of instances to allocate. Can be at most 65535. */
    maxCount: number;
    /** Mesh resource. */
    mesh: Mesh | null;
    /** Material resource. */
    material: Material | null;
    /** Whether to use per-particle colors. */
    colors: boolean;
    /** Whether to use per-particle instance data. */
    instanceData: boolean;
}
/**
 * Wrapper around a particle effect resource.
 *
 * For more information about how to create particle effects, have a look at
 * the {@link ParticleEffectManager} class.
 *
...
export declare class ParticleEffect extends Resource {
     ...
     *
     * It is best practice to set the particle effect variable to `null` after calling
     * destroy to prevent accidental use:
     *
     * ```js
     ...
     * ```
     *
     * Accessing the particle effect after destruction behaves like accessing
     * an empty particle effect.
     */
    destroy(): void;
    toString(): string;
}
 ...
 *
 * @note Probe volume scenarios are **per-engine**, they can thus be shared by
 * multiple scenes.
 *
 * @since 1.5.0
 */
export declare class ProbeVolumeScenario extends Resource {
    /** @hidden */
    static getResourceType(): ResourceType | null;
    get loaded(): boolean;
}
 ...
 *
 * For more information about how to create textures, have a look at the
...
export declare class AnimationGraph extends SceneResource {
     ...
     *
     * @note The index of the sampler is the same as
     * the index of the animation in the graph.
     *
     * #### Usage
     *
     * Following example creates a graph with 1D blending, but reuses
     * the forward walk animation as backwards walk by setting the playbackspeed
     * of its sampler to -1.
     * ```js
     ...
     * ```
     *
     * @param samplerIndex Index of the sampler
     * @param speed Playback speed to set
     * @experimental
     */
...
export declare class Environment {
    ...
    /**
     * Request a re-bake of the environment.
     *
     * @note Currently, baking only takes into account the sky material.
     *

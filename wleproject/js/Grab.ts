...
export class GrabComponent extends Component {
  ...
  soundUnClick: AudioSource;
  mesh: MeshComponent;
  defaultMaterial: any;
  hoverMaterial: any;
  buttonMeshObject: any;
  realParent: Object3D;
  oldTransform: Float32Array = new Float32Array(16);
  ...
  start() {
    ...
    this.realParent = this.object.parent;
  ...
  /* Called by 'cursor-target' */
  onDown = (_, cursor: Cursor) => {
    this.soundClick.play();
    //this.buttonMeshObject.translate([0.0, -0.1, 0.0]);
    hapticFeedback(cursor.object, 1.0, 20);

    this.object.getTransformWorld(this.oldTransform);

    this.object.parent = cursor.object;
    this.object.setTransformWorld(this.oldTransform);
  };
  /* Called by 'cursor-target' */
  onUp = (_, cursor) => {
    ...
    this.object.getTransformWorld(this.oldTransform);
    this.object.parent = this.realParent;
    this.object.setTransformWorld(this.oldTransform);
  };
